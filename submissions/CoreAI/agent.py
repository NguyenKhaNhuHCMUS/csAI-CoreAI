...
class Seeker:
    ...
    def a_star_intercept(self, start, ghost_pos, last_ghost_pos, map_state, pacman_speed):
        target = ghost_pos
        
        if last_ghost_pos:
            dr, dc = ghost_pos[0] - last_ghost_pos[0], ghost_pos[1] - last_ghost_pos[1]
            for k in range(2, 0, -1):
                predict = (ghost_pos[0] + dr * k, ghost_pos[1] + dc * k)
                if 0 <= predict[0] < map_state.shape[0] and 0 <= predict[1] < map_state.shape[1]:
                    if map_state[predict] == 0:
                        target = predict
                        break
        ...
        while frontier:
            ...
            for neighbor, move in Utils.get_valid_neighbors(current, map_state):
                ...
                if neighbor not in visited or new_cost < visited[neighbor]:
                    ...
                    h = Utils.manhattan_dist(neighbor, target) / max(1, pacman_speed)
                    heapq.heappush(frontier, (new_cost + h, neighbor, path + [move]))
        return []
    ...
    def find_patrol_target(self, my_pos, map_state):
        ...
        while queue:
            ...
            v = self.visit_count.get(curr, 0)
            if v < min_visits:
                min_visits = v
                best_target = curr

            for neighbor, _ in Utils.get_valid_neighbors(curr, map_state):
...
class MyCoreAgent:
    ...
    def solve(self, map_state, my_pos, enemy_pos, step_number):
        ...
        if self.role == "seeker":
            # Cập nhật dấu chân cho tuần tra
            self.seeker_logic.visit_count[my_pos] = self.seeker_logic.visit_count.get(my_pos, 0) + 1
            
            # 1. Xác định mục tiêu
            target = enemy_pos
            if target is None:
                target = self.seeker_logic.find_patrol_target(my_pos, map_state)
            
            # 2. Tìm đường bằng A* 
            path = self.seeker_logic.a_star_intercept(
                my_pos, target, self.last_known_enemy_pos, map_state, self.pacman_speed
            )
            
            # Cập nhật vị trí cuối cùng nhìn thấy
            if enemy_pos is not None:
                self.last_known_enemy_pos = enemy_pos

            # 3. Trả về nước đi tối ưu (1 hoặc 2 ô)
